...
from auth_service.domain.utils import (
    ensure_aware,
    ensure_not_none,
)
...
class VerificationCode:
    ...
    def __init__(
        ...
        self._user_public_id: uuid.UUID = VerificationCode._validate_user_public_id(
            user_public_id
        )

        self._type: CodeType = VerificationCode._validate_type(type)

        self._created_at: datetime = VerificationCode._validate_created_at(created_at)
    ...
    def _validate_not_before_created_at(self, at: datetime, field: str) -> datetime:
    ...
    def _validate_type(code_type: CodeType) -> CodeType:
        ...
        if not isinstance(code_type, CodeType):
            raise TypeError(
                f'Invalid code type: expected CodeType, got {type(code_type).__name__}'
            )
